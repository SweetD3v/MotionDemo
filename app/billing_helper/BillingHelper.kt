...
import com.interactivewhiteboard.blackboard.utils.ensureMainThread
...
class BillingHelper(
    ...
    val jobPurchase = Job()
    val ioScope = CoroutineScope(Dispatchers.IO + jobPurchase)
    ...
    fun queryProduct(productId: String, successDialog: Boolean? = false) {
        ...
        ioScope.launch {
            ...
            billingClient = BillingClientSetup.getInstance(
                activity
    ...
    fun launchBillingFlow() {
        if (activity.isOnline()) {
            billingClient?.let {
                if (it.isReady) {
                    BillingFlowLauncher.launchBillingFlow(
                        activity,
                        productDetailsList,
                        it,
